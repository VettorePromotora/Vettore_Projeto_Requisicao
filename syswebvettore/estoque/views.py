...
from .models import (Estoque, EstoqueEntrada, EstoqueItens, EstoqueSaida, ProtocoloEntrega)
from Usuarios.models import Perfil

from Cadastro.models import Local, Categoria
...
class EstoqueEntradaList(ListView):
    ...
    def get_queryset(self):
        ...
        return EstoqueEntrada.objects.filter(funcionario=user)
...
def dar_baixa_estoque(form):
    ...
    for item in produtos:
        ...
        produto.estoque_atl = item.saldo
        produto.save()
...
def dar_baixa_no_estoque_com_protocolo_de_entrega(request, pk):
    ...
    for item in entrega.protocolo_entrega.all():
        ...
        EstoqueItens.objects.create(
            ...
            saldo=item.produto.estoque_atl - item.quantidade
        )
...
def exportar_csv(request):
    header = ('estoque_local', 'nome', 'categoria_f_m', 'est_inic', 'est_min',
              'descricao', 'unid_medida', 'cod_produto', 'estoque_atl')

    produtos = Produtos.objects.all().values_list(*header)
    path = "Downloads"
    with open(os.path.join(path, 'syswebvettorepromotora.csv'), 'w') as csvfile:
        ...
        for produto in produtos:
            ...
            produto_list[0] = Local.objects.filter(pk=produto[0])
            produto_list[0] = produto_list[0][0]
            produto_list[2] = Categoria.objects.filter(pk=produto[2])
            produto_list[2] = produto_list[2][0]

            produto_write.writerow(produto_list)
        messages.success(request, 'Download realizado com sucesso')
        return HttpResponseRedirect(reverse('listar-produtos'))
